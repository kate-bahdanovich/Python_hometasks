...
from content_from_json import ReadContentFromJson
from content_from_xml import ReadContentFromXml
...
source_of_input = user_input.read_correct_char(("0", "1", "2", "3"),
                                               "choose source of import: 0 for txt file, 1 for console, 2 for json file, 3 for xml file: ")

if source_of_input == "0":                                                                  # new part of 'main' code for handling input from file
    source_file = ReadContentFromTxt(handling_types=("News", "Advert", "UserComment"))      # create object to read file input
...
# new part of 'main' code for handling input from file "json" or "xml"
elif source_of_input in ("2", "3"):
    if source_of_input == "2":
        source_file = ReadContentFromJson(handling_types=("News", "Advert", "UserComment")) # create object to read input from json
    elif source_of_input == "3":
        source_file = ReadContentFromXml(handling_types=("News", "Advert", "UserComment"))  # create object to read input from xml
    list_of_draft_publications = source_file.read_content()                                 # read file content, clean data, normalize 'text' field, return list to a variable
    for draft_publication in list_of_draft_publications:                                    # loop through a list of data suitable for publication creating
        if draft_publication['type'] == "News":
            publ = content.News(draft_publication)                                          # if choice = 1 => create News content from 'draft_publication'
        elif draft_publication['type'] == "Advert":
            publ = content.Advert(draft_publication)                                        # if choice = 2 => create Adv content from 'draft_publication'                                                                    # skip this publication
        elif draft_publication['type'] == "UserComment":
            publ = content.UserComment(draft_publication)                                   # if choice = 3 => create UserComment content from 'draft_publication'
        if publ.mistakes_flag == 1:                                                         # if we find an incorrect date while 'publ' is created
            source_file.set_successful_processing()                                         # set flag 'file successful processing' to false (default value for procedure)
            continue                                                                        # skip current line from farther processing
        publ.create_publication_body()                                                      # form publication body for each content type
        publ.write_content()                                                                # write publication to a file
        # add data to letter-csv and words-csv files
        csv_data = ContentParsingToCsv()
