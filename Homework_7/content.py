...
class News(Content):                                                        # define news content type
    def __init__(self, values_source=[]):                                   # by default object will be created from console input
        Content.__init__(self)                                              # define fields from parent class
        if values_source == []:                                             # create object from console(default)
            ...
            self.text = values_source[1]                                    # set text
            self.input_type = 'file'                                        # set input_type
...
class Advert(Content):                                                      # define private advert type
    def __init__(self, values_source=[]):                                   # by default object will be created from console input
        Content.__init__(self)                                              # define fields from parent class
        self.mistakes_flag = 0                                              # as object contains date fields it is needed to check their correctness
        if values_source == []:                                             # create object from console(default)
    ...
    def create_publication_body(self):                                      # form publication body
        ...
        if self.mistakes_flag == 0:                                         # we can handle incorrect dates by skipping such values. If dates are correct
            ...
            self.pub_body = f"{self.text} \nActual until: {self.exp_date.strftime('%d/%m/%Y')}, {self.num_of_days} days left"   # form advertising body with date and number_of_days
